...
const GameSession = () => {
    ...
    const placeBets = () => {
        if ((userInfo.balance-bet)===0) {
            ...
            setUserInfo({
                ...
                uuid: userInfo.isLoggedIn,
                balance: userInfo.isLoggedIn,
                gameID: userInfo.gameSession['_id'],
                gameSession: userInfo.gameSession
            });
            // refresh();
    ...
    const [bet, setBet] = useState(userInfo.balance);
            ...
            <div className="blackjackBoard">           
                {/* <div className="betRow"> */}
                    ...
                    <div className="bettingPool">
                        <body>Bet: ${userInfo.balance-bet}</body>
                        {userInfo.gameSession['canLeave'] && <button onClick={placeBets}>Submit</button> }
                        {userInfo.gameSession['canLeave'] && <button onClick={() => setBet(userInfo.balance)}>Clear</button>}
                    </div>

                    <div className="currentBalance">
                        <body>
                            Balance: ${userInfo.balance-(userInfo.balance-bet)}
                        </body>
                    </div>
